...
# ===== 类别ID → 类别名称（和你 det.txt 一致）=====
CLASS_NAMES = {
    0: "Pedestrian",
    1: "Cyclist",
    2: "Vehicles",   # Car/Truck/Van 合并后的类
}
def color_by_id(tid: int):
    """
    给每个 track_id 分配一个稳定颜色（BGR），同一 ID 永远同色。
    """
    tid = int(tid)
    # 用一个简单 hash，让颜色分布更均匀、且稳定
    r = (tid * 37) % 255
    g = (tid * 17) % 255
    b = (tid * 29) % 255
...
DETS_ROOT = r"E:\track\dataset\BusFrontMOT\dets_stnms_X_all"

# 轨迹与可视化保存目录（相对 BoT-SORT 主目录）
RESULTS_ROOT = r"E:/track/BoT-SORT-main/YOLOX_outputs/m2la_botsort_dets_stnms_all"
...
# 类别映射（与你的 det.txt / 训练类别一致）
CLASS_NAME = {0: 'Pedestrian', 1: 'Cyclist', 2: 'Vehicles'}
...
def track_one_sequence(seq_name):
    ...
    while True:
        ...
        for t in online_targets:
            ...
            online_clses.append(tcls)

            # 保存到结果 txt:
            # frame, id, x, y, w, h, score, -1, -1, -1
            f_res.write(
                f"{frame_id},{tid},{tlwh[0]:.2f},{tlwh[1]:.2f},"
                f"{tlwh[2]:.2f},{tlwh[3]:.2f},{tscore:.2f},{tcls},-1,-1\n"
            )

        # 可视化
        if SAVE_VIS:
            # --- Custom visualization (avoid double text from plot_tracking) ---

            online_im = frame.copy()

            for tlwh, tid, tscore, tcls in zip(online_tlwhs, online_ids, online_scores, online_clses):
                ...
                cls_name = CLASS_NAMES.get(int(tcls), "Unknown")
