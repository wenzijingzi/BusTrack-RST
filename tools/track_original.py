...
def make_parser():
    ...
    parser.add_argument(
        ...
        help=(
            "cmc method: "
            "files (Vidstab GMC) | sparseOptFlow | orb | ecc | none | busfront"
        ),
    )
        ...
        if frame_id % 20 == 0:
            logger.info(
                'Processing frame {}/{} ({:.2f} fps)'.format(
                    frame_id, num_frames, 1. / max(1e-5, timer.average_time)
                )
            )
